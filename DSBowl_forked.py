# -*- coding: utf-8 -*-
"""
Created on Sat Feb 17 09:04:03 2018
...
img_df['ImageId'] = img_df.path.str.split('\\').str.get(3)
img_df['ImageType'] = img_df.path.str.split('\\').str.get(4)
img_df['Train_or_Test'] = (img_df.path.str.split('\\').str.get(2)).str.split('_').str.get(1)
img_df['Stage'] = (img_df.path.str.split('\\').str.get(2)).str.split('_').str.get(0)
...
train_df = img_df.query('Train_or_Test=="train"')
train_rows = []
...
reduce_lr_loss = ReduceLROnPlateau(monitor='loss', factor=0.05, patience=5, verbose=1, epsilon=1e-4, mode='min')
...
simple_cnn.load_weights(filepath = '.mdl_wts.hdf5')

test_df = img_df.query('Train_or_Test=="test"')
test_rows = []
group_cols = ['Stage', 'ImageId']
